import { Class, Generalization, Diagram, Project, ClassStereotype, Relation, RelationStereotype } from '@libs/ontouml';
// GeneralizationSet, ModelElement,
...
export class Abstractor implements Service {
  ...
  abstract(rule: string): Module {
    ...
    switch (rule) {
      case 'RelatorAbstraction':
        abstraction = this.relatorAbstraction();
        break;
      case 'NonSortalAbstraction':
        abstraction = this.nonSortalAbstraction();
        break;
      case 'SortalAbstraction':
        abstraction = this.sortalAbstraction();
        break;
      case 'SubkindAndPhasePartitionsAbstraction':
        abstraction = this.subkindAndPhasePartitionsAbstraction();
        break;
    }
   ...
   */
  relatorAbstraction(): Module {
    const abstraction = new Module('Relator Abstraction');
    ...
    relations.forEach(relation => {
        if (!relation.isDerived) {
            console.log(relation.getSourceClass().name.textMap.get('en'));
            console.log(relation.getTargetClass().name.textMap.get('en'));
            console.log("----");
        } else {
  ...
  copyRelationFromSourceToTarget(source: Class, target: Class, relation: Relation) {
    switch (relation.stereotype) {
      case RelationStereotype.MATERIAL:
        this.project.model.createMaterialRelation(source, target, relation.getName() + 'as' + source.getName());
        break;
      case RelationStereotype.COMPARATIVE:
        this.project.model.createComparativeRelation(source, target, relation.getName() + 'as' + source.getName());
        break;
      case RelationStereotype.MEDIATION:
        this.project.model.createMediationRelation(source, target, relation.getName() + 'as' + source.getName());
        break;
      case RelationStereotype.CHARACTERIZATION:
        this.project.model.createCharacterizationRelation(source, target, relation.getName() + 'as' + source.getName());
        break;
      case RelationStereotype.COMPONENT_OF:
        this.project.model.createComponentOfRelation(source, target, relation.getName() + 'as' + source.getName());
        break;
      case RelationStereotype.MEMBER_OF:
        this.project.model.createMemberOfRelation(source, target, relation.getName() + 'as' + source.getName());
        break;
      case RelationStereotype.SUBCOLLECTION_OF:
        this.project.model.createSubCollectionOfRelation(source, target, relation.getName() + 'as' + source.getName());
        break;
      case RelationStereotype.SUBQUANTITY_OF:
        this.project.model.createSubQuantityOfRelation(source, target, relation.getName() + 'as' + source.getName());
        break;
      case RelationStereotype.INSTANTIATION:
        this.project.model.createInstantiationRelation(source, target, relation.getName() + 'as' + source.getName());
        break;
      case RelationStereotype.TERMINATION:
        this.project.model.createTerminationRelation(source, target, relation.getName() + 'as' + source.getName());
        break;
      case RelationStereotype.PARTICIPATIONAL:
        this.project.model.createParticipationalRelation(source, target, relation.getName() + 'as' + source.getName());
        break;
      case RelationStereotype.PARTICIPATION:
        this.project.model.createParticipationRelation(source, target, relation.getName() + 'as' + source.getName());
        break;
      case RelationStereotype.HISTORICAL_DEPENDENCE:
        this.project.model.createHistoricalDependenceRelation(source, target, relation.getName() + 'as' + source.getName());
        break;
      case RelationStereotype.CREATION:
        this.project.model.createCreationRelation(source, target, relation.getName() + 'as' + source.getName());
        break;
      case RelationStereotype.MANIFESTATION:
        this.project.model.createManifestationRelation(source, target, relation.getName() + 'as' + source.getName());
        break;
      case RelationStereotype.BRINGS_ABOUT:
        this.project.model.createBringsAboutRelation(source, target, relation.getName() + 'as' + source.getName());
        break;
      case RelationStereotype.TRIGGERS:
        this.project.model.createTriggersRelation(source, target, relation.getName() + 'as' + source.getName());
        break;
    }
  }
   ...
   */
  nonSortalAbstraction(): Module {
    const abstraction = new Module('Non Sortal Abstraction');

    const nonSortals = this.project
      .getAllClassesByStereotype(ClassStereotype.ROLE_MIXIN)
      .concat(
        this.project
          .getAllClassesByStereotype(ClassStereotype.MIXIN)
          .concat(this.project.getAllClassesByStereotype(ClassStereotype.CATEGORY)),
      );
      ...
      for (let j = 0; j < nonSortals.length; j++) {
        ...
        if (genereralizations[i].getGeneral() == nonSortals[j]) {
          ...
          for (let x = 0; x < genereralizations[i].getSpecifics().length; x++) {
            this.copyRelationsFromGeneralToSpecificList(
              genereralizations[i].getGeneralClass(),
              genereralizations[i].getSpecificClasses()[x],
            );
          }
        }
      }
   ...
   */
  sortalAbstraction(): Module {
    const abstraction = new Module('Sortal Abstraction');

    const allClasses = this.project.getAllClasses();

    const sortals = this.project
      .getAllClassesByStereotype(ClassStereotype.KIND)
      .concat(
        this.project
          .getAllClassesByStereotype(ClassStereotype.SUBKIND)
          .concat(
            this.project
              .getAllClassesByStereotype(ClassStereotype.ROLE)
              .concat(
                this.project
                  .getAllClassesByStereotype(ClassStereotype.PHASE)
                  .concat(
                    this.project
                      .getAllClassesByStereotype(ClassStereotype.COLLECTIVE)
                      .concat(this.project.getAllClassesByStereotype(ClassStereotype.QUANTITY)),
                  ),
              ),
          ),
      );
    ...
    for (let i = 0; i < generalizations.length; i++) {
        ...
        if (generalizations[i].getGeneralClass() == sortals[j] && generalizations[i] != null) {
          ...
          for (let w = 0; w < relations.length; w++) {
            if (relations[w].getSourceClass() == generalizations[i].getSpecificClass()) {
              this.copyRelationFromSpecificToGeneral(
                generalizations[i].getSpecificClass(),
                generalizations[i].getGeneralClass(),
                relations[w],
                true,
              );
            } else {
      ...
      for (let z = 0; z < relations.length; z++) {
        ...
        if (
          relations[z].getSourceClass() == generalizations[i].getSpecificClass() ||
          relations[z].getTargetClass() == generalizations[i].getSpecificClass()
        ) {
  ...
  subkindAndPhasePartitionsAbstraction(): Module {
    const abstraction = new Module('Subkind And Phase Partitions Abstraction');
    ...
    for (let i = 0; i < generalizationSets.length; i++) {
      if (generalizationSets[i].isPhasePartition() || generalizationSets[i].isSubkindPartition()) {
        ...
        for (let j = 0; j < generalizationSets[i].getSpecificClasses().length; j++) {
          ...
          for (let w = 0; w < relations.length; w++) {
            if (relations[w].getSourceClass() == generalizationSets[i].getSpecificClasses()[j]) {
              this.copyRelationFromSpecificToGeneral(
                generalizationSets[i].getSpecificClasses()[j],
                generalizationSets[i].getGeneralClass(),
                relations[w],
                true,
              );
        ...
        /**5 */
        this.project.model.createBinaryRelation(
          generalizationSets[i].getGeneralClass(),
          this.project.model.createEnumeration(generalizationSets[i].getName(), itens),
        );
      }
    }
